...
const getFilmsByTitle = async (title, ctx) => {
    ...
    if(searched_film) {
        // if found films are less than 5 take that number, otherwise 6
        let length = (searched_film.length < 6) ? searched_film.length : 6;
        ...
        ctx.answerInlineQuery(films, {cache_time: 0});
    }
}
