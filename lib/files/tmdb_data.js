...
const list = {
    // add a new user request to list
    // if > 20, remove first 5 elements of list
    add(idUser, type){
        if(data.list.length > 20){
            data.list.splice(0, 5);
        }
        data.list.push({id: idUser, type: type});
    },

    // retun true if there is a user
    // remove user form list
    findByUserIdAndRemove(idUser){
        let found = false;

        for(let i = 0; i < data.list.length; i++){
            if(data.list[i].id == idUser){
                found = true;
