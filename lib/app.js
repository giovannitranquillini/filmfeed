...
const { send, update } = require('./telegram/telegramMethods');
    ...
    (query.getUserNumber()).then((users) => {
        if(users) {
            const message = 'Le informazioni sui film sono fornite da [TMDb](https://www.themoviedb.org)\n\n' +
                    'Il codice del bot potrebbe essere rilasciato in futuro sul mio [github](https://github.com/giovannitranquillini/)\n\n' +
...
bot.help(ctx => {
    ...
    const message ='*Lasciati aiutare da FilmFeed!* \n' +
                    ...
                    'Per avere maggiori informazioni usa /info';
    send(id, message, {parse_mode : 'Markdown'});
});
